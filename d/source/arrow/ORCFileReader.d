...
public class ORCFileReader : ObjectG
{
	...
	public this(SeekableInputStream file)
	{
		...
		auto __p = garrow_orc_file_reader_new((file is null) ? null : file.getSeekableInputStreamStruct(), &err);
		...
		this(cast(GArrowORCFileReader*) __p, true);
	}
	...
	public int[] getFieldIndexes()
	{
		...
		auto __p = garrow_orc_file_reader_get_field_indexes(gArrowORCFileReader, &nFieldIndexes);

		return __p[0 .. nFieldIndexes];
	}
	...
	public int[] getFieldIndices()
	{
		...
		auto __p = garrow_orc_file_reader_get_field_indices(gArrowORCFileReader, &nFieldIndices);

		return __p[0 .. nFieldIndices];
	}
	...
	public RecordBatch readStripe(long i)
	{
		...
		auto __p = garrow_orc_file_reader_read_stripe(gArrowORCFileReader, i, &err);
	...
	public Table readStripes()
	{
		...
		auto __p = garrow_orc_file_reader_read_stripes(gArrowORCFileReader, &err);
	...
	public Schema readType()
	{
		...
		auto __p = garrow_orc_file_reader_read_type(gArrowORCFileReader, &err);
