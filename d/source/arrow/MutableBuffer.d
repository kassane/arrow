...
public class MutableBuffer : Buffer
{
	...
	{
		auto __p = garrow_mutable_buffer_new(data.ptr, cast(long)data.length);
		...
		this(cast(GArrowMutableBuffer*) __p, true);
	}
	...
	{
		auto __p = garrow_mutable_buffer_new_bytes((data is null) ? null : data.getBytesStruct());
	...
	public bool setData(long offset, ubyte[] data)
	{
		...
		auto __p = garrow_mutable_buffer_set_data(gArrowMutableBuffer, offset, data.ptr, cast(long)data.length, &err) != 0;
	...
	{
		auto __p = garrow_mutable_buffer_slice(gArrowMutableBuffer, offset, size);
		...
		return ObjectG.getDObject!(MutableBuffer)(cast(GArrowMutableBuffer*) __p, true);
	}
}
