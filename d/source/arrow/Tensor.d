...
public class Tensor : ObjectG
{
	 ...
	 */
	public this(DataType dataType, Buffer data, long[] shape, long[] strides, string[] dimensionNames)
	{
		auto __p = garrow_tensor_new((dataType is null) ? null : dataType.getDataTypeStruct(), (data is null) ? null : data.getBufferStruct(), shape.ptr, cast(size_t)shape.length, strides.ptr, cast(size_t)strides.length, Str.toStringzArray(dimensionNames), cast(size_t)dimensionNames.length);
		...
		this(cast(GArrowTensor*) __p, true);
	}
	...
	{
		auto __p = garrow_tensor_get_buffer(gArrowTensor);
	...
	public long[] getShape()
	{
		...
		auto __p = garrow_tensor_get_shape(gArrowTensor, &nDimensions);

		return __p[0 .. nDimensions];
	}
	...
	public long[] getStrides()
	{
		...
		auto __p = garrow_tensor_get_strides(gArrowTensor, &nStrides);

		return __p[0 .. nStrides];
	}
	...
	{
		auto __p = garrow_tensor_get_value_data_type(gArrowTensor);
